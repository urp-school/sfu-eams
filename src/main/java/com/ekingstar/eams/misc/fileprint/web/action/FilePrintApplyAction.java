//$Id: FilePrintApplyAction.java,v 1.21 2006/11/29 01:31:31 duanth Exp $
/*
 *
...
public class FilePrintApplyAction extends CalendarRestrictionSupportAction {
	...
	public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		query.add(new Condition(" filePrintApplication.calendar.id=:calendarId", calendarId));
		query.add(new Condition(" filePrintApplication.applyBy=:applyBy", getUser(request
				.getSession())));
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (!"".equals(filePrintApplicationId)) {
			FilePrintApplication filePrintApplication = (FilePrintApplication) utilService.get(
					FilePrintApplication.class, Long.valueOf(filePrintApplicationId));
	...
	public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		FilePrintApplication filePrintApplication = (FilePrintApplication) populateEntity(request,
				FilePrintApplication.class, "filePrintApplication");
		DynaActionForm dynaForm = (DynaActionForm) form;
		FormFile formFile = (FormFile) dynaForm.get("formFile");

		// 判断是否已有上传文档, 有则删除
		if (null != filePrintApplication.getId()) {
			FilePrintApplication application = (FilePrintApplication) utilService.load(
					FilePrintApplication.class, filePrintApplication.getId());
			if (null != application.getFilePath()) {
				SystemConfig config = SystemConfigLoader.getConfig();
				String defaultPath = request.getSession().getServletContext().getRealPath(
						FilePath.fileDirectory);
				File file = new File(FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
						+ application.getFilePath());
				log.info("user [" + getUser(request.getSession()).getName() + "] delete file["
		...
		if (!"".equals(formFile.getFileName())) {
			String filePath = System.currentTimeMillis()
					+ formFile.getFileName().substring(formFile.getFileName().lastIndexOf("."));
			try {
				...
				OutputStream bos = new FileOutputStream(FilePath.getRealPath(config, FilePath.DOC,
						defaultPath)
	...
	public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (int i = 0; i < filePrintApplicationIds.length; i++) {
			...
			FilePrintApplication filePrintApplication = (FilePrintApplication) utilService.load(
					FilePrintApplication.class, filePrintApplicationId);
			SystemConfig config = SystemConfigLoader.getConfig();
			String defaultPath = request.getSession().getServletContext().getRealPath(
					FilePath.fileDirectory);
			File file = new File(FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
					+ filePrintApplication.getFilePath());
			log.info("user [" + getUser(request.getSession()).getName() + "] delete file["
					+ file.getAbsolutePath() + "]");
	...
	public ActionForward download(ActionMapping mapping, ActionForm form,
		...
		if (!file.exists()) {
			response.getWriter().write(
					"without file path:["
							+ FilePath.getRealPath(config, FilePath.DOC, defaultPath)
							+ "/"
							+ new String(filePrintApplication.getFilePath().getBytes(),
									"ISO-8859-1") + "]");
			return null;
		}
