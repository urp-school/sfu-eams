...
public class TaskSeqNoByCourseGenerator extends BasicHibernateDAO implements TaskSeqNoGenerator {
    ...
    public void genTaskSeqNo(TeachTask task) {
        ...
        String courseCode = task.getCourse().getCode();
        synchronized (this) {
            String hql = "select seqNo from TeachTask  where calendar.id="
                    + task.getCalendar().getId() + " and course.id=" + task.getCourse().getId()
                    + " and seqNo is not null order by seqNo";
            ...
            for (Iterator iter = taskSeqNos.iterator(); iter.hasNext();) {
                ...
                if (StringUtils.isEmpty(courseCode)) {
                    courseCode = StringUtils.substringBeforeLast(seqNo, ".");
                }
                seqNo = StringUtils.substringAfterLast(seqNo, ".");
            ...
            newSeqNo++;
            task.setSeqNo(courseCode + "."
                    + StringUtils.repeat("0", 2 - String.valueOf(newSeqNo).length()) + newSeqNo);
        }
    }
