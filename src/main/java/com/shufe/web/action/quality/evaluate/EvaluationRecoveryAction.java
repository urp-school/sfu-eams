 ...
 * @author hj,chaostone
 */
public class EvaluationRecoveryAction extends CalendarRestrictionSupportAction {
    ...
    public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        request.setAttribute("stdTypes", getStdTypesOf(stdTypeId, "questionnaireRecycleRate",
                request));
    ...
    public ActionForward doStatisticInCourseTask(ActionMapping mapping, ActionForm form,
        ...
        Map recycleMap = new HashMap();
        List recycleObjectList = questionnaireRecycleRateService.getRecycleRateObject(
                departmentIds, studentTypeIds, teachCalendars);
        for (Iterator iter = recycleObjectList.iterator(); iter.hasNext();) {
            QuestionnaireRecycleRate element = (QuestionnaireRecycleRate) iter.next();
            recycleMap.put(element.getDepartment().getId() + "-" + element.getStudentType().getId()
                    + "-" + element.getTeachCalendar().getId(), element);
        }
        ...
        for (Iterator iter = statisticRecycleRateList.iterator(); iter.hasNext();) {
            ...
            QuestionnaireRecycleRate recycleObject = (QuestionnaireRecycleRate) recycleMap
                    .get(department.getId() + "-" + stdType.getId() + "-" + teachCalendar.getId());
        ...
        return redirect(request, "search", "", parameters);
    }
    ...
    public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        if (StringUtils.isNotBlank(query) && "query".equals(query)) {
            String requireDeprtIdSeq = request.getParameter("teachDepartId");
            if (StringUtils.isBlank(requireDeprtIdSeq)) {
                requireDeprtIdSeq = departmentIds;
            }
            String requireStdTypeIdSeq = request.getParameter("stdTypeId");
            String year = request.getParameter("year");
            String term = request.getParameter("term");
            String teachCalendarId = request.getParameter("teachCalendarId");
            String isAllType = request.getParameter("isAllType");
            ...
            if (StringUtils.isNotBlank(teachCalendarId)) {
                ...
                teachCalendar = teachCalendarService.getTeachCalendar(Long
                        .valueOf(requireStdTypeIdSeq), year, term);
                if (null == teachCalendar) {
                    teachCalendar = teachCalendarService
                            .getNearestCalendar((StudentType) EntityUtils.getEntity(
                                    StudentType.class, Long.valueOf(requireStdTypeIdSeq)));
                }
            }
            if ("true".equals(isAllType)) {
                requireStdTypeIdSeq = stdTypeIdSeq;
            }
            ...
            List recycleRateList = questionnaireRecycleRateService.getRecycleRateObject(
                    requireDeprtIdSeq, requireStdTypeIdSeq, teachCalendars);
            if (recycleRateList.size() > 0) {
                Results.addList("recycleRateList", recycleRateList);
            } else {
                return forward(mapping, request, "field.recycleRate.errors", "finish");
            }
            ...
            List departments = departmentService.getDepartments(departmentIds);
