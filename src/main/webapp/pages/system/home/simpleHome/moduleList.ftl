  ...
  <tr>
   <td>
	  <SCRIPT language="javascript">
		...
		menuTree.iconFolderCollapsed = iconPath+'/images/tree/entityfolder.gif';
		menuTree.iconFolderExpanded = iconPath+'/images/tree/entityfolder.gif';
		<#assign rootCode='0'>
		...
		<#if module.entry?default("")=='home.do?method=welcome'><#else>
		 ...
		 <#if (module.entry?exists&&module.entry?length!=0)>
		     ...
		     menuTree.node${module.code} = menuTree.makeItem("<@i18nNameTitle module/>",'${moduleEntry}','<#if moduleEntry?starts_with("http://")>_blank<#else>main</#if>','',"<@i18nNameTitle module/>", 'javascript:menuTree.nodeClickedAndSelected(${module_index + 1})');
