...
<table id="teacherUserBar"></table> 
 <table width="70%"  class="formTable" align="center">
  <form name="teacherUserForm" action="teacherUser.do?method=save" method="post" onsubmit="return false;">
   <@searchParams/>
   <tr class="darkColumn">
     <td  colspan="2"><@bean.message key="info.user.teacher"/></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td ><input type="password" name="password" value="" id="firstPassword" maxlength="20"/></td>
     <input type="hidden" name="user.password" value=""/></td>
   </tr>
   <tr>
  ...
  <script language="javascript" >
   function save(form){
     ...
     if (v.exec()) {
       if(form.firstPassword.value!=form.secondPassword.value) {
        form.firstPassword.select();
        form.secondPassword.value="";
        alert("<@bean.message key="info.password.notSame"/>");
        return;
        }else{
          form['user.password'].value=hex_md5(form.firstPassword.value);          
          form.submit();
        }
     }
   }
   var bar = new ToolBar('teacherUserBar','<@bean.message key="info.userUpdate.teacher"/>',null,true,true);
