	...
	<table id="backBar"></table>
     <table class="infoTable">
	   ...
	   <tr>
	     ...
	     <td class="content" width="20%"><a href="javascript:courseInfo()" title="点击课程名称查看，该课程详细信息及简介。"><@i18nName task.course/></a></td>
      ...
      <form method="post" action="" name="actionForm">
      	<input type="hidden" name="id" value="${task.course.id}"/>
      	<input type="hidden" name="type" value="Course"/>
      </form>
	<script>
		var bar = new ToolBar('backBar','<@bean.message key="entity.teachTask"/><@bean.message key="common.detailInfo"/>',null,true,true);
		bar.setMessage('<@getMessage/>');
		bar.addItem("查看课程信息","courseInfo()", "detail.gif");
