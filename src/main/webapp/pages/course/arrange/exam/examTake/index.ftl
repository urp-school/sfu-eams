...
<BODY>
	<table id="taskBar"></table>
     <table class="frameTable_title">
      <tr>
       <td>详细查询</td>
	    <form name="takeSearchForm" method="post" action="examTake.do?method=index" action="return false;">
 ...
 <script>
	  ...
	  bar.addItem("应考统计","statTake()");
  	  bar.addItem("未参加考试学生名单","stdList('noTakeStdList')");
  	  bar.addItem("冲突学生名单","stdList('collisionStdList')");
	  bar.addHelp("<@msg.message key="action.help"/>");
	  function stdList(method){
        form.target="contentFrame"
	    form.action="examTake.do?method="+method;
	    form.submit();
	  }
	  function search(pageNo,pageSize,orderBy){
	    form.target="contentFrame";
		form.action="examTake.do?method=examTakeList";
	    goToPage(form,pageNo,pageSize,orderBy);
	  }
	  function statTake(){
	    form.target="_blank";
		form.action="examTake.do?method=statTake";
		form.submit();
	  }
	  search();
 </script>
