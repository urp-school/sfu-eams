...
<table id="teachClassStdBar"></table>
 <@table.table width="100%" id="listTable">
    ...
    <form name="tmpForm" method="post" action="courseTakeForTask.do?method=stdList&taskId=${task.id}&pageSize=70">
      <input name="adminClassName" type="hidden" value=""/>
    </form>
</div>
   <#include "addStdForm.ftl"/>
  <script>
   var bar = new ToolBar('teachClassStdBar','<@msg.message key="attr.taskNo"/>[${task.seqNo}],学生名单(${task.teachClass.courseTakes?size})人',null,true,true);
  ...
  function displayDiv(divId){
	...
	function withdraw(){
     	submitId(document.stdListForm, "courseTakeId", true, "courseTakeForTask.do?method=withdraw&log=1","确认你所的学生要退课？");
	}
  function printStdListForDuty(){
     window.open("teachTask.do?method=printStdListForDuty&teachTaskIds=${task.id}");
  }
