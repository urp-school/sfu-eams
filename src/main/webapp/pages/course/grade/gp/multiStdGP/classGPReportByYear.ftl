     ...
     <table width="100%" align="center" class="listTable">
	   <tr class="title" align='center'>
	     ...
	     <#if year!=calendar.year>
		     <td >${calendar.year} 学年度</td>
		 </#if>    
	     <#assign year = calendar.year/>
	     </#list>
	   </tr>
	   <#list stdGPList as stdGP>
	    <tr align="center">
		   ...
		   <#assign stdGp = 0/>
		   <#list calendars as calendar>	   
		   <#if stdGpYear==calendar.year>
		   <td><#if stdGP.getGP(calendar)?exists><@reserve2  stdGP.getGP(calendar)+stdGp/></#if></td>
		   <#else>
		   <#assign stdGp = stdGP.getGP(calendar)?if_exists/>
		   </#if>
		   <#assign stdGpYear = calendar.year/>
		   </#list>
	   </#list>
     </table>
  </#list>
