...
<body  onload="SetPrintSettings()">
   <#list examTasks?if_exists as examTask >
   <#assign boycount=0/>
   <#assign girlcount=0/>
    <#assign examTake=examTask?sort_by(['std','code'])>
    <br>
	 ...
	 <#assign pageNos=(examTake?size/(pagePrintRow*2))?int />
	 <#if ((examTake?size)>(pageNos*(pagePrintRow*2)))>
	 ...
	 <tr class="infoTitle">
		   ...
		   <td ><@msg.message key="attr.courseNo"/>：${examTake[0]?if_exists.task.course.code}</td>
		   <td ><@msg.message key="attr.courseName"/>：<@i18nName examTake[0]?if_exists.task.course?if_exists/></td>
		   <td ><@msg.message key="attr.teachDepart"/>：<@i18nName examTake[0]?if_exists.task.arrangeInfo.teachDepart/></td>
	 </tr>
	 </table>
	 <table class="printTableStyle"  width="100%"  >
	   ...
	   <tr class="brightStyle" >
         <#if examTake[i+passNo]?exists>
	     <td>${i+1+passNo}</td>
	     <td>${examTake[i+passNo]?if_exists.std?if_exists.code}</td>
	     <td><@i18nName examTake[i+passNo]?if_exists.std?if_exists/></td>
	     <td><@i18nName examTake[i+passNo]?if_exists.examType/></td>
	     <@emptyTd count=2/>
         </#if>
         
         <#if examTake[i+pagePrintRow+passNo]?exists>
	     <td>${i+pagePrintRow+1+passNo}</td>
	     <td>${examTake[i+pagePrintRow+passNo]?if_exists.std?if_exists.code}</td>
	     <td><@i18nName examTake[i+pagePrintRow+passNo]?if_exists.std?if_exists/></td>
	     <td><@i18nName examTake[i+pagePrintRow+passNo]?if_exists.examType/></td>
	     <@emptyTd count=2/>
         <#elseif examTake[i+passNo]?exists>
          <@emptyTd count=6/>
         </#if>
