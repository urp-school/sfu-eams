    ...
    <form name="stdSearch" method="post" action="" onsubmit="return false;">
    	...
    	<tr>
	     ...
	     <td>
	      <input type="text" name="courseGrade.std.code" maxlength="32" size="10" value="${RequestParameters['student.code']?if_exists}" style="width:100px;"/>
	     </td>
		</tr>
    	<tr>
	     <td><@msg.message key="attr.personName"/>:</td>
	     <td>
	      <input type="text" name="courseGrade.std.name" maxlength="20" size="10" value="${RequestParameters['student.name']?if_exists}" style="width:100px;"/>
	     </td>
		</tr>
	   <tr>
	     <td>就读年级:</td>
      ...
      <tr>
    	...
    	<tr>
	     ...
	     <td>
	      <input type="text" name="courseGrade.taskSeqNo" maxlength="32" value="${RequestParameters["courseGrade.taskSeqNo"]?if_exists}" style="width:100px;"/>
         </td>
        </tr>
    	<tr>
	     <td><@msg.message key="attr.courseNo"/>:</td>
	     <td>
	      <input type="text" name="courseGrade.course.code" maxlength="32" value="${RequestParameters["courseGrade.course.code"]?if_exists}" style="width:100px;"/>
         </td>
        </tr>
    	<tr>
	     <td><@msg.message key="attr.courseName"/>:</td>
	     <td>
	      <input type="text" name="courseGrade.course.name" maxlength="20" value="${RequestParameters["courseGrade.course.name"]?if_exists}" style="width:100px;"/>
         </td>
        </tr>
    	<tr>
	     <td><@msg.message key="entity.courseType"/>:</td>
	     <td>
	      <input type="text" name="courseGrade.courseType.name" maxlength="20" value="${RequestParameters["courseGrade.courseType.name"]?if_exists}" style="width:100px;"/>
         </td>
        </tr>
        <tr>
	     <td>状态:</td>
    	...
    	<tr>
	     <td>分数范围:</td>
	     <td><input name="scoreFrom" value="${RequestParameters["scoreFrom"]?if_exists}" maxlength="3" style="width:40px"/>-<input name="scoreTo" maxlength="3" value="${RequestParameters["scoreTo"]?if_exists}" style="width:40px"/></td>
        </tr>
       <tr>
	     <td>是否通过:</td>
	     <td>
       ...
       <tr>
		<td id="f_studentStatus">学籍状态:</td>
	 	<td>
	 		<select name="courseGrade.std.state.id" style="width:100px;">
				<option value="">...</option>
				<#list result.studentStateList as studentState>
					<option value="${studentState.id}"<#if RequestParameters["courseGrade.std.state.id"]?default("0")==studentState.id?string> selected</#if>/><@i18nName studentState/></option>
				</#list>
			</select>
	 	</td>
	  </tr>
