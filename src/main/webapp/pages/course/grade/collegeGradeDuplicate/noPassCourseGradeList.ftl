...
<BODY LEFTMARGIN="0" TOPMARGIN="0">
 ...
 <@table.table width="100%" sortable="true" id="sortTable">
	   ...
	   <@table.tbody datas=noPassCourseGrades;grade>
        ...
        <td>${grade.taskSeqNo}</td>
        <td>${grade.course.code}</td>
        <td><@i18nName grade.course/></td>
        <td>${grade.getScoreDisplay()}</td>
	   </@>
     </@>
  <form name="actionForm" method="post" action="collegeGradeDuplicate.do?method=noPassCourseGradeList" onsubmit="return false;">
      <input name="calendar.id" type="hidden" value="${RequestParameters['calendar.id']}">
  </form>
  <script>
    var bar = new ToolBar("gradeListBar","不及格成绩查询结果（实践）",null,true,true);
    ...
    function exportData() {
    	form.action = "collegeGradeDuplicate.do?method=export";
