...
<BODY>
	<table id="taskBar"></table>	
     <table class="frameTable_title">
      <tr>
       ...
       <td>|</td>
      <form name="taskForm" target="teachTaskListFrame" method="post" action="collegeGradeDuplicate.do?method=index" onsubmit="return false;">
  ...
  <table>
 <script>
	var bar=new ToolBar("taskBar","录入教学任务成绩（实践）",null,true,true);
   ...
   function searchTask(pageNo,pageSize,orderBy){
     form.target="teachTaskListFrame";
	    taskForm.action="collegeGradeDuplicate.do?method=search";
     goToPage(form,pageNo,pageSize,orderBy);
   }
   function noGradeTakes(){
       form.action="collegeGradeDuplicate.do?method=noGradeTakeList";
       form.submit();
       }
   function noPassCourseGrades(){
       form.action="collegeGradeDuplicate.do?method=noPassCourseGradeList";
       form.submit();
       }
 </script>
</body>
