...
<body>
 ...
 <#include "../taskListTable.ftl"/>
  ...
  <script language="JavaScript" type="text/JavaScript" src="scripts/course/grade/gradeDuplicate.js"></script>
  <script>
  	 var action="${action}";
     var bar=new ToolBar('taskBar','实践课程的教学任务',null,true,true);
     bar.setMessage('<@getMessage/>');
     bar.addItem('查看成绩','info(document.actionForm)');
     bar.addItem('成绩状态','editGradeStateInfo(document.actionForm)');
     ...
     gradeStateMenu.addItem("批量设置录入方式","batchUpdateGradeState()",'update.gif');
