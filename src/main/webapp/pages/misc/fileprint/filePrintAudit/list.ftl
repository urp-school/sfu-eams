...
<@table.table id="listTable" width="100%" sortable="true">
  <@table.thead>
    ...
    <@table.tbody datas=filePrintApplicationList;filePrintApplication>
     ...
     <td><a href='filePrintAudit.do?method=download&filePrintApplicationId=${(filePrintApplication.id)?if_exists}'>${(filePrintApplication.fileName)?if_exists}</a></td>
  ...
  <@htm.actionForm name="actionForm" entity="filePrintApplication" action="filePrintAudit.do">
...
<script>
    var bar=new ToolBar("filePrintApplicationBar","请印申请列表",null,true,true);
  	...
  	bar.addItem("审核通过","audit('pass')");
  	bar.addItem("取消审核通过","audit('unPass')");
    bar.addItem("<@bean.message key="action.add"/>","add()");
  	...
  	function audit(method) {
	    ...
	    form.action = "filePrintAudit.do?method=" + method;
