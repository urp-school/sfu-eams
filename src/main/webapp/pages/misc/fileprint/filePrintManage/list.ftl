...
<@table.table id="listTable" width="100%" sortable="true">
  <@table.thead>
    ...
    <@table.tbody datas=filePrintApplicationList;filePrintApplication>
     ...
     <td><a href='filePrintManage.do?method=download&filePrintApplicationId=${(filePrintApplication.id)?if_exists}'>${(filePrintApplication.fileName)?if_exists}</a></td>
  ...
  <@htm.actionForm name="actionForm" entity="filePrintApplication" action="filePrintManage.do">
...
<script>
    var bar=new ToolBar("filePrintApplicationBar","请印申请列表",null,true,true);
  	...
  	bar.addItem("耗材明细","material()");
  	bar.addItem("计算总费用","filePrint('computeTotal')");
  	bar.addItem("设置为未完成","filePrint('openFilePrint')");
  	bar.addItem("设置为已完成","filePrint('closeFilePrint')");
  	
  	function filePrint(method) {
  		var ids = getSelectIds("filePrintApplicationId");
	 	if (ids == null || ids == "") {
	 		alert("你没有选择要操作的记录！");
	 		return;
	 	}
	 	
	    form.action = "filePrintManage.do?method=" + method;
	    addParamsInput(form, resultQueryStr);
	    submitId(form, 'filePrintApplicationId', true);
  	}
  	function material() {
	 	var ids = getSelectIds("filePrintApplicationId");
	 	if (ids == null || ids == "") {
	    ...
	    form.action = "filePrintManage.do?method=materialList";
	    addParamsInput(form, resultQueryStr);
	    submitId(form, 'filePrintApplicationId', false);
	}
</script>
