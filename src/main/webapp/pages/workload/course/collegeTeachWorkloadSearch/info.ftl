 ...
 <script>
 	function collegeAffirm(){
 		...
 		if(confirm("你确认要确认这个工作量吗?")){
 			document.teachWorkloadForm.action="collegeWorkloadQuery.do?method=affirmSelect&affirmType=collegeAffirm&affirmEstate=true";
 	...
 	function teacherAffirm(){
 		...
 		if(confirm("你确认要确认这个工作量吗?")){
    		document.teachWorkloadForm.action="collegeWorkloadQuery.do?method=affirmSelect&affirmType=teacherAffirm&affirmEstate=true";
    		setSearchParams(parent.form,document.teachWorkloadForm);
    		document.teachWorkloadForm.submit();
    	}
 	}
