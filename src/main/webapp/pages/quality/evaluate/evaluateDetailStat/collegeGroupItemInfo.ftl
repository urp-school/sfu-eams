...
<body>
    <table id="bar"></table>
    <#assign pageCaption = "本次课程质量评价全校分项统计汇总表"/>
    <#assign tdCount = 5/>
    ...
    <table width="${tableWidth}px" align="center">
        <tr>
            <td style="font-size:13.5pt;font-weight:bold;text-align:center">${pageCaption}</td>
        </tr>
    </table>
    <table class="listTable" width="${tableWidth}px" align="center">
        ...
        <#assign typeTotal = 0/>
        <#list collegeResults?keys as dKey>
            <#if typeTotal != 0 && typeTotal != collegeResults[dKey][5]>
        <tr>
            <td colspan="7" style="font-size:10pt;color:#FF6600;font-weight:bold;font-family:宋体,MiscFixed">好：${(typeTotal * 0.9)?string("0.#")}分以上；较好：${(typeTotal * 0.8)?string("0.#")}--${(typeTotal * 0.9 - 0.1)?string("0.#")}分；中：${(typeTotal * 0.7)?string("0.#")}--${(typeTotal * 0.8 - 0.1)?string("0.#")}分；较差：${(typeTotal * 0.6)?string("0.#")}--${(typeTotal * 0.7 - 0.1)?string("0.#")}分；差：${(typeTotal * 0.6 - 0.1)?string("0.#")}分以下</td>
        </tr>
            </#if>
            <#assign typeTotal = collegeResults[dKey][5]/>
        <tr>
            <td rowspan="2">${questionTypeMap[dKey].name}（${typeTotal}分）</td>
            <td style="color:red">人次</td>
            <#assign cTotal = 0/>
            <#list 1..tdCount as i>
                <#assign cTotal = cTotal + collegeResults[dKey][i - 1]/>
            <td>${collegeResults[dKey][i - 1]}</td>
            </#list>
        </tr>
        <tr>
            <td>学校比例（%）</td>
