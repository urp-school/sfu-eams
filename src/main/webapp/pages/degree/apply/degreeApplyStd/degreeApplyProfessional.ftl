...
<body>
	...
	<h2 align="center">授予专业硕士学位基本数据表</h2>
	<table width="100%" class="listTable">
		<tr>
			<td rowspan="2" align="center">学<br>科<br>信<br>息</td>
			<td width="12%">授学位单位</td>
			<td colspan="3">${(systemConfig.school.name)?if_exists}</td>
			<td width="12%">获学位类别</td>
		...
		<tr>
			<td rowspan="3" align="center">学<br>业<br>信<br>息</td>
			<td>学号</td>
			<td>${(student.code)?if_exists}</td>
			<td>入学日期</td>
			<td>${(student.studentStatusInfo.enrollDate)?if_exists}</td>
			<td>毕业日期</td>
			<td>${(student.degreeInfo.graduateOn)?if_exists}</td>
			<td width="12%">获硕士学位日期</td>
		...
		<tr>
			...
			<td colspan="3"></td>
			<td>学位证书号</td>
			<td colspan="3">${(student.degreeInfo.certificateNo)?if_exists}</td>
		</tr>
		<tr>
			...
			<td colspan="4">${(student.firstMajor.name)?if_exists}</td>
		</tr>
		<tr>
			<td>授学士学位单位</td>
			<td>${(student.degreeInfo.undergraduate.school.name)?if_exists}</td>
		...
		<tr>
			<td rowspan="3" align="center">其<br>他<br>信<br>息</td>
			<td>工作单位</td>
	...
	<script>
	   ...
	   <#if "std" == flag?if_exists>
		   bar.addItem("下载个人学位数据表", "downloadTemplate('degreeInfo', 'P')");
