...
<script>
   var bar = new ToolBar('backBar','申请学位－<@i18nName systemConfig.school/>申请硕士学位数据表',null,true,true);
   bar.setMessage('<@getMessage/>');
   <#if "std"==flag?if_exists>
   bar.addItem("下载个人学位数据表","downloadTemplate('degreeInfo','M')");
   bar.addItem("<#if (degreeApply.id)?exists>重新申请<#else>申请学位</#if>",'doSubmit(document.dataInfoForm)');
   bar.addPrint("<@msg.message key="action.print"/>");
   </#if>
   ${otherJS?if_exists}
</script>
<h2 align="center">授予学历硕士学位基本数据表</h2>
<table width="100%" class="listTable">
	...
	<tr>
		<td rowspan="2" align="center">其<br>他<br>信<br>息</td>
