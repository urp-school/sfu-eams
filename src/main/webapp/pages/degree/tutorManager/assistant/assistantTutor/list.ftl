...
<@htm.actionForm name="actionForm" entity="assistant" action="assistantTutor.do"/>   
    <script> 
    var bar = new ToolBar("assistantBar","助教管理",null,true,true);
    bar.setMessage('<@getMessage/>');
    bar.addItem("<@msg.message key="action.confirm"/>",'confirm(1)');  
    bar.addItem("取消确认",'confirm(0)');  
    function confirm(isConfirm){
       var form=document.actionForm;
       addInput(form,"isConfirm",isConfirm);
       multiAction("confirm");
    }
    </script>
