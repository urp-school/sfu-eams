   ...
   <tr>
    <td>
     <table width="90%" align="center" class="listTable">
	   ...
	   <#list result.specialitAspectsList?if_exists as specialitAspect>
	   ...
	   <tr class="${class}">
	    ...
	    <td width="5%" align="center" bgcolor="#CBEAFF">
	     <input type="radio" name="specialitAspectId" value="${specialitAspect.id}" <#if RequestParameters.specialityAspectId?exists && RequestParameters.specialityAspectId==specialitAspect.id?string>checked</#if> />
	    </td>
   ...
   <tr>
    <td align="center" colspan="3" >
     <input type="button" name="button1" value="<@bean.message key="system.button.confirm" />" class="buttonStyle"
            onClick="window.top.opener.setSpecialityAspectIdAndDescriptions(getId('specialitAspectId'),getName(getId('specialitAspectId')));
                     window.close();"  />&nbsp;&nbsp;&nbsp;&nbsp;
     <input type="reset" name="reset" value="<@bean.message key="system.button.reset" />" class="buttonStyle" 
            onClick="window.top.opener.setSpecialityAspectIdAndDescriptions('', '');window.top.opener.resetSpecialityAspectsSelector();
                     window.close();"/>
    </td>
   </tr>
  </table>
