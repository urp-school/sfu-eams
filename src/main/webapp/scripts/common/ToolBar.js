...
var imagePath="http://static.urp.sfu.edu.cn/eams/1.0/images/action/";
...
function addBack(msg){
   if(null==msg){
      this.addItem("返回",function (){history.back()},'backward.gif');
   }else{
      this.addItem(msg,function (){history.back()},'backward.gif');
   }
}
function addHelp(title, module){
   this.addItem((title == null || title == "") ? "帮助" : title,function (){
...
function addMenu(title,action,imageName,alt){
   ...
   if(null!=action){
	  ...
	  setAction(itemTd,action);
      downarrow.onclick=function (event){MCH(event,menu_str);adaptFrameSize();};
	  ...
	  itemTd.onclick=function (event){MCH(event,menu_str);adaptFrameSize();};
   }
   this.itemCount++;
...
function addBackOrClose(backCaption, closeCaption) {
	if (parent.location == self.location && (window.history.length <= 1 || null == window.history.length)) {
