	...
	/**
	 * 判断是否相同的活动
	 * same acitivity [teacherId,courseId,roomId,vaildWeeks]
	 */
	...
	/**
	 *合并相同的教学活动
	 *same [teacherId,courseId,roomId] can merge
	 */
	function canMergeWith(other){
	   return (this.teacherId==other.teacherId&&this.courseId==other.courseId&&this.roomId==other.roomId);	   
	}
	...
	function repeatChar(str,length){
   	...
   	/** 
   	 * 添加缩略表示
   	 *  add a abbreviation to exists result;
   	 *  Do not use it directly. a white space will delimate the weeks
   	 *  For example:odd1-18 even3-20
   	 */
    ...
    /** aggreagate activity of same course.
     *  first merge the activity of same (teacher,course,room).
     *  then output mashal vaildWeek string .
     *  if course is null. the course name will be ommited in last string.
     *  style is --------------------------------
     *          | teacher1Name course1Name       |
     *          | (odd1-2,room1Name)             |
     *          | (even2-4,room2Name)            |
     *          | teacher2Name course1Name       |
     *          | (odd3-6,room1Name)             |
     *          | (even5-8,room2Name)            |
     *          ----------------------------------
     * @param index time unit index 
     * @param from  start position in year occupy week
     * @param startWeek   bengin position from [from]
     * @param endWeek end position from [from]
     */
    function marshalByTeacherCourse(index,from,startWeek,endWeek){
    	...
    	if(this.activities[index].length==1){  
    	  ...
    	  return tname+" "+cname +delimiter+"(" +this.activities[index][0].adjustClone(this.endAtSat,validStart,false).marshal(from,startWeek,endWeek)+")";
    	    ...
    	    // merge this.activities to tempActivities by same courseName and roomId .start  with 1.
    ...
    /**
     * 根据该年份是否结束于星期六，调整教学州的占用串。
     * 如果没有调整则返回原来的activity.否则返回调整后的新的activity。
     * @activity 要调整的教学活动
    ...
    //new taskAcitvity
	function TaskActivity(teacherId,teacherName,courseId,courseName,roomId,roomName,vaildWeeks,taskId){
   	...
   	function cloneTaskActivity(){
   	    return new TaskActivity(this.teacherId,this.teacherName,this.courseId,this.courseName,this.roomId,this.roomName,this.vaildWeeks,this.taskId);
   	}
   	// 
    ...
    /************************************************************************
    * course table dispaly occupy of teacher,room and andminClass.        
    * It also represent data model of any course arrangement. For
    * example student's course table,single course's table,teacher's
    * course table,and adminClass's course table,even speciality's .
    ************************************************************************/
    function CourseTable(year,unitCounts){
        ...
        // return true,if this.activities[first] and this.activities[second] has same vaildWeeks and roomId pair set.
    ...
    ************************************************************************/
    ...
    /* construct a valid Weeks from this.weeksMap by key teacherRoomId
     * this startweek is the position of this.weeksMap[teacherRoomId] in return validWeekStr
     * also it has mininal value 1;
     */
    function constructValidWeeks(startWeek,teacherRoomId){
	    ...
	    //alert(weeksThisYear)
	    var validWeeks=new Array();
	    
	    if(weeksThisYear.indexOf("1")!=-1){
 	       validWeeks[validWeeks.length]=weeksThisYear;
	    }
	    var weeksNextYear="";
	    if(null!=secondWeeks) {
	        for(i=0;i<weeksPerYear;i++){
	        ...
	        if(weeksNextYear.indexOf("1")!=-1){
	           validWeeks[validWeeks.length]=weeksNextYear;
	        }
	        //alert(weeksNextYear);
	    }
	    //alert(validWeeks)
    ...
    function constructActivities(startWeek){
        ...
        for(var teacherRoomId in this.activityMap){
           var validWeeks=this.constructValidWeeks(startWeek,teacherRoomId);
           this.activityMap[teacherRoomId].vaildWeeks=validWeeks[0];
           ...
           if(validWeeks.length==2){
             ...
             cloned.vaildWeeks=validWeeks[1];
	 ...
	 */ 
    function ActivityCluster(year,courseId,courseName,weeks){
        this.year=year;
        var date = new Date();
        date.setFullYear(year,11,31);
        this.endAtSat=false;
