...
//         var now = new Date(new Date().valueOf() - nowLength);
    ...
    function canElect(state,task){
        ...
        if(null!=state.hisCourses[task.course.id]){
           if(state.params.isRestudyAllowed){
              /*if(state.hisCourses[task.course.id]){
                 return errors.reStudyPassedCourseNotAllowed;
              }else{
     ...
     */
     ...
     function addTask(){
        ...
        tr.onclick=function (){onRowChange(getEvent());};
        /*table.tBodies[table.tBodies.length-1].appendChild(tr); user this next version*/
    ...
    function cancelTask(stdCode){
        ...
        if(taskId!==""&& confirm("确定要退掉这门课吗？")){
           ...
           var inputDiv =selectInput.parentNode.parentNode.cells[0];
           if(inputDiv.firstChild && inputDiv.firstChild.value=="false"){
