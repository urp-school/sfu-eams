    ...
    function gradeStateInfo(form){
	 ...
	 function editGradeStateInfo(form){
	    submitId(form,"taskId",false,"courseGrade.do?method=editGradeState");
	 }
	 function publishGrade(form,gradeTypeId,isPublished,allGradeType){
	    setSearchParams();
	    //alert(form['params'].value);
	    if(isPublished==1) submitId(form,"taskId",true,"courseGrade.do?method=publishGrade&isPublished="+isPublished+"&allGradeType="+allGradeType,"确定要发布成绩吗?");
	    if(isPublished==0) submitId(form,"taskId",true,"courseGrade.do?method=publishGrade&isPublished="+isPublished+"&allGradeType="+allGradeType,"确定要取消发布吗?");
	 }